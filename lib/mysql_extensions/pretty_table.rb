module PrettyTable
  # Prints nice-looking plain-text tables via puts
  # 
  #   +--+-------+
  #   |id|name   |
  #   |--+-------|
  #   |1 |fasdfas|
  #   |2 |test   |
  #   +--+-------+
  
  # records is an array of hashes
  # columns is an array of the columns, strings or symbols
  def self.lines(records, columns = nil)
    if columns
      columns.map!{|x| x.to_s }
    else
      columns = records.first.keys.sort_by{|x|x.to_s}
    end
    
    sizes = column_sizes(records, columns)
    sep_line = separator_line(columns, sizes)
    
    l = []
    
    l << sep_line
    l << header_line(columns, sizes)
    l << sep_line
    records.each {|r| l << data_line(columns, sizes, r)}
    l << sep_line
    l
  end
  
  def self.print(records, columns=nil)
    lines(records, columns).each do |line|
      puts line
    end
  end
  
  ### Private Module Methods ###

  # Hash of the maximum size of the value for each column 
  def self.column_sizes(records, columns) # :nodoc:
    sizes = Hash.new {0}
    columns.each do |c|
      s = c.to_s.size
      sizes[c.to_s] = s if s > sizes[c.to_s]
    end
    records.each do |r|
      columns.each do |c|
        s = r[c].to_s.size
        sizes[c.to_s] = s if s > sizes[c.to_s]
      end
    end
    sizes
  end
  
  # String for each data line
  def self.data_line(columns, sizes, record) # :nodoc:
    '|' << columns.map {|c| format_cell(sizes[c], record[c])}.join('|') << '|'
  end
  
  # Format the value so it takes up exactly size characters
  def self.format_cell(size, v) # :nodoc:
    case v
    when Bignum, Fixnum
      "%#{size}d" % v
    when Float
      "%#{size}g" % v
    else
      "%-#{size}s" % v.to_s
    end
  end
  
  # String for header line
  def self.header_line(columns, sizes) # :nodoc:
    '|' << columns.map {|c| "%-#{sizes[c]}s" % c.to_s}.join('|') << '|'
  end

  # String for separtor line
  def self.separator_line(columns, sizes) # :nodoc:
    '+' << columns.map {|c| '-' * sizes[c]}.join('+') << '+'
  end
end
